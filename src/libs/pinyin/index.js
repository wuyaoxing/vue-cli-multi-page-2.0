...
pinyinEngine.toPinyin = function() {
	...
	return function(keyword, single, sp, isjx) {
		...
		if (len === 1) {
			...
			if (isjx) {
				for (var i = 0; i < pys.length; i++) {
					...
					for (var m = 0; m < pys[i].length; m++) {
						jx = jx + pys[i][m].charAt(0);
					}
				}
				return jx;
			}
...
/*
  张中海
*/
pinyinEngine.search = function(data, val, field) {
	if (!val || !data)
		return data;
	val = val.trim().toLowerCase();
	if (!val)
		return data;
    var pyStr = pinyinEngine.toPinyin(val, true, '', '');
	if (!pyStr)
		return data;
	...
	return data.filter(function(item) {
		var isFinded = false;
		var _fieldValue = item[field].trim();
		if (!_fieldValue)
			return false;
		_fieldValue = _fieldValue.toLowerCase();
		if (_fieldValue.indexOf(val) != -1) {
			return true;
		}
		var pys = pinyinEngine.toPinyin(_fieldValue, false, null);
		var jxArr = [];
		var pyArr = [];
		for (var i = 0; i < pys.length; i++) {
			var jx = "";
			...
			pyArr = pyArr.concat(pys[i]);
			jxArr.push(jx)
			pyArr.push(Array.isArray(pys[i]) ? pys[i].join('') : pys[i]);
		}
		_.each(jxArr, function(v, k) {
			if (v.indexOf(pyStr) != -1) {
				isFinded = true;
				return false;
			}
		})
		if (!isFinded)
			_.each(pyArr, function(v, k) {
				if (v.indexOf(pyStr) == 0) {
					isFinded = true;
					return false;
				}
			})
