<template>
    <div class="epm-calendar"
         :class="{'epm-calendar-landspace': mode === 'landscape'}">
        <!-- <date-display @selectYear="selectYear"
                                                      @selectMonth="selectMonth"
                                                      :monthDaySelected="displayMonthDay"
                                                      :disableYearSelection="disableYearSelection"
                                                      :selectedDate="selectedDate"
                                                      :dateTimeFormat="dateTimeFormat"></date-display> -->
        <div class="epm-calendar-container">
            <div class="epm-calendar-monthday-container"
                 v-if="displayMonthDay">
                <calendar-toolbar :slideType="slideType"
                                  :nextMonth="nextMonth"
                                  :prevMonth="prevMonth"
                                  @monthChange="handleMonthChange"
                                  :displayDates="displayDates"
                                  :dateTimeFormat="dateTimeFormat" />
                <div class="epm-calendar-week">
                    <span class="epm-calendar-week-day"
                          v-for="weekText, index in weekTexts"
                          :key="index">{{weekText}}</span>
                </div>
                <div class="epm-calendar-monthday">
                    <transition :name="'epm-calendar-slide-' + slideType"
                                v-for="displayDate, index in displayDates"
                                :key="index">
                        <div class="epm-calendar-monthday-slide"
                             :key="displayDate.getTime()">
                            <calendar-month @selected="handleSelected"
            ...
            <calendar-year @change="handleYearChange"
                           v-if="!displayMonthDay"
                           :selectedDate="selectedDate"
                           :maxDate="maxDate"
                           :minDate="minDate" />
            <div class="epm-calendar-actions">
                <!-- <flat-button :label="cancelLabel"
                                                                 @click="handleCancel"
                                                                 primary/>
                                                    <flat-button v-if="!autoOk"
                                                                 @click="handleOk"
                                                                 :label="okLabel"
                                                                 primary/> -->
            </div>
    ...
    methods: {
        ...
        handleOk () {
            ...
            if (selectedDate.getTime() > maxDate.getTime()) {
                this.selectedDate = new Date(maxDate.getTime())
            }
            if (selectedDate.getTime() < minDate.getTime()) {
                this.selectedDate = new Date(minDate.getTime())
            }
        ...
        changeDislayDate (date) {
            ...
            if (date.getFullYear() === oldDate.getFullYear()
                && date.getMonth() === oldDate.getMonth()) return
...
.epm-calendar-slide-next-enter-active,
.epm-calendar-slide-next-leave-active,
.epm-calendar-slide-prev-enter-active,
.epm-calendar-slide-prev-leave-active {
